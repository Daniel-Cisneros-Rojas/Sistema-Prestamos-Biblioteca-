...
if ($id_libro === null) {
  echo "<div style='text-align:center; margin-top:20px;' class='alert alert-danger'>No se ha proporcionado un ID de libro.</div>";
  exit();
}
          ...
          <div class="col-md-8">
            <div class="bg-light p-4 rounded shadow-sm">
              ...
              if ($resultado->num_rows > 0) {
                  ...
                  if ($libro['estado'] == 'no_disponible') {
                      echo "<div class='alert alert-warning'>El libro no está disponible para préstamo.</div>";
                      exit();
                  }

                  echo "<h4 class='mb-3'>Solicitar préstamo de: <strong>" . $libro['titulo'] . "</strong></h4>";
                  echo "<p>Autor: <strong>" . $libro['autor'] . "</strong></p>";
                  echo "<p>Cliente: <strong>" . $_SESSION['nombre'] . "</strong></p>";
                  echo "<form action='procesar_prestamo.php' method='post'>";
                  echo "<input type='hidden' name='id_libro' value='" . $id_libro . "'>";
                  echo "<p>Fecha inicio préstamo: <strong>" . date("Y-m-d") . "</strong></p>";
                  echo "<div class='form-group'>";
                  echo "<label for='dias'>Número de días:</label>";
                  echo "<input type='number' name='dias' class='form-control' placeholder='Número de días' required pattern='[0-9]+' min='1' max='7'>";
                  echo "</div>";
                  echo "<button type='submit' class='btn btn-primary mt-3'>Solicitar préstamo</button>";
				  if (isset($_SESSION['mensaje_prestamo'])) {
    				$tipo = $_SESSION['mensaje_prestamo']['type'];
    				$texto = $_SESSION['mensaje_prestamo']['text'];
    				echo "<div class='alert alert-$tipo mt-3'>$texto</div>";
    				unset($_SESSION['mensaje_prestamo']);
					}
                  echo "</form>";
              } else {
                  echo "<div class='alert alert-danger'>No se encontró el libro.</div>";
                  exit();
              }
              ?>
	         </div>
