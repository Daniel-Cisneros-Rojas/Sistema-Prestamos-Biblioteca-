...
$stmt_id_usuario = $mysqli->prepare("SELECT id FROM usuarios WHERE user = ?");
if ($stmt_id_usuario) {
    $stmt_id_usuario->bind_param("s", $usuario);
    $stmt_id_usuario->execute();
    $res_id_usuario = $stmt_id_usuario->get_result();

    if ($res_id_usuario->num_rows === 1) {
        $datos = $res_id_usuario->fetch_assoc();
        $id_usuario = $datos['id'];
    }
...
if (is_null($id_usuario)) {
    echo "Error: No se pudo verificar la identidad del usuario.";
    exit();
}
...
if ($stmt === false) {
    die("Error al preparar la consulta de libros: " . $mysqli->error);
}
          ...
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav  ">
              ...
              if (isset($_SESSION['autenticado']) && $_SESSION['autenticado'] === true) {
                  ...
                  if ($_SESSION['tipo'] === 'user') {
                      echo '<li class="nav-item active"><a class="nav-link" href="usr_libro.php">Mis libros</a></li>';
                  }
              } else {
    ...
    <div class="service_container">
      <div class="container ">
		...
		if (isset($_SESSION['msg'])) {
    		echo "<div class='alert alert-info text-center mt-3'>" . htmlspecialchars($_SESSION['msg']) . "</div>";
    		unset($_SESSION['msg']);
		}
		?>
          <?php
          ...
          if ($resultado->num_rows > 0) {
              ...
              echo "<thead class='bg-primary text-white'><tr><th>ID</th><th>Portada</th><th>Título</th><th>Autor</th><th>Año</th><th>Editorial</th><th>Descripción</th><th>Estado</th><th>Propietario</th><th class='acciones'>Acciones</th></tr></thead><tbody>";
              while ($row = $resultado->fetch_assoc()) {
                  echo "<tr>";
                  echo "<td>" . htmlspecialchars($row['id']) . "</td>";
                  echo "<td>";
                  if (!empty($row['imagen']) && file_exists('./' . $row['imagen'])) {
                      echo "<img src='./" . htmlspecialchars($row['imagen']) . "' alt='Portada'>";
                  } else {
                      echo "<img src='./imagenes/no_disponible.png' alt='No Disponible' width='80'>";
                  }
                  echo "</td>";
                  ...
                  echo "<a href='editar_libro.php?id=" . htmlspecialchars($row['id']) . "'>Editar</a> ";
                  echo "<a href='eliminar_libro.php?id=" . htmlspecialchars($row['id']) . "' onclick='return confirm(\"¿Estás seguro de que deseas eliminar este libro?\");'>Eliminar</a>";
                  echo "</td>";
                  echo "</tr>";
              }
              echo "</tbody></table>";
          } else {
              echo "<p>No tienes libros registrados en el sistema.</p>";
          }
          $resultado->free();
          $mysqli->close();
          ?>
